...
用“有冲突的研究评审”逼出更好的科研决策，但冲突必须服务于判断，不服务于表演。每次参加会议的成员，都知道其他成员的存在。
...
**成员感知规则**:
- `Moderator` 在 `Roundtable Setup` 中必须列出 `Active Roles`
- 每个成员默认知道其他在场成员是谁，也知道前面成员已经说过什么
- 回应时优先直接点角色名，不要匿名地说“有人认为”
...
- 每个角色至少给出 1 个明确判断，可以是支持、限缩、保留或反对
- 每次支持、限缩、保留或反对都要给出理由，最好附带修复路径或验证动作
...
- `Debate Round`: 存在关键分歧时，设置 `Lead`、`Opponent`、`Reply`、`Judge`
- `Alignment Round`: 暂时不存在实质反方时，设置 `Lead`、`Support or Qualify`、`Judge`
- 不要为了形式硬造反方；只有真的存在关键分歧时，才进入正反交锋
- 即使没有反方，也要说明“为什么暂时没有反方”以及“还剩什么未验证风险”

### 每次后手发言都必须点名回应

每个后手发言都要明确回答以下至少一项：
- 你反驳的是哪位角色的哪一句核心判断
- 你赞同的是哪位角色的哪一点，并补充什么边界或保留条件
- 你接受对方哪一部分，但不同意哪一部分
- 你认为哪一个证据或假设会改变结论
...
必须像这样互动：
- A 提命题
- B 直接回应 A，可以是反驳、支持或限缩
- A 或 C 继续处理 B 提出的具体点
- `Moderator` 或 `Judge` 裁定当前到底卡在什么证据或方法上

### 回合结束条件取决于实际情况

如果这一轮存在实质分歧，至少要出现 1 次“来回”：
- `主张 -> 反驳 -> 回应`

如果这一轮不存在实质分歧，至少要出现 1 次“对前文的明确支持或限缩”：
- `主张 -> 支持/限缩 -> 裁定`
...
每一轮开始前，`Moderator` 必须把当前议题改写成一个单句命题，再判断这一轮是 `Debate Round` 还是 `Alignment Round`。
...
每轮只打 1 个核心议题，不要散；如果是 `Debate Round` 却没有来回交锋，或者是 `Alignment Round` 却没有明确支持/限缩，也不算完成这一轮。

**每轮必须包含**:
1. `Moderator` 先声明本轮命题、轮次类型和参与角色名
2. `Lead` 先给出主张，限制在 `1-2` 句
3. 如果是 `Debate Round`：
   - `Opponent` 必须直接反驳 `Lead` 的某个具体点
   - `Reply` 必须对刚才那条反驳作答，不能转移到新议题
4. 如果是 `Alignment Round`：
   - `Support or Qualify` 必须明确说明自己为什么不反驳 `Lead`
   - 同时补充边界、残余风险或需要继续验证的点
5. `Judge` 明确指出：当前谁占优，或者为什么目前没有实质反方；还缺什么证据；是否继续下一轮
6. 至少 1 个可执行动作：修复、验证、降调、停损或转向
7. 本轮结束时给出收敛度：`高 / 中 / 低`

**发言检查**:
- 如果一句话删掉前文仍然成立，说明它不是回应，必须重写
- 如果某角色没有引用前一个角色或更早一位角色的 claim、证据或假设，说明它在自说自话
...
## Round 1（命题：...，轮次类型：Debate / Alignment，收敛度：低/中/高）
### Moderator
> 本轮只讨论这个命题：...
> 参与角色：ResearchArchitect、RedTeamReviewer、Methodologist
> 本轮判断：存在 / 不存在 实质反方

### ResearchArchitect（Lead）
> 我的主张是...

### RedTeamReviewer（Opponent / Support or Qualify）
> 我反对 / 我不反对 `ResearchArchitect` 刚才关于“...”的判断，因为...

### Methodologist（Reply / Support or Qualify）
> 我接受 / 不接受 `RedTeamReviewer` 对 `ResearchArchitect` 的这条意见，因为...

### Moderator（Judge）
> 当前占优的是...；或者目前没有实质反方；真正未解决的是...；如果要翻盘或推进，需要补...
